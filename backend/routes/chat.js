...
router.post('/delete-conversations', auth, async (req, res) => {
  try {
    const { conversationIds } = req.body;
    const userId = req.user.id;
    
    // Delete all messages between user and selected partners
    await Message.deleteMany({
      $or: conversationIds.flatMap(partnerId => [
        { senderId: userId, recipientId: partnerId },
        { senderId: partnerId, recipientId: userId }
      ])
    });
...
router.post('/delete-messages', auth, async (req, res) => {
  try {
