...
router.put('/:id/accept', auth, async (req, res) => {
  try {
    ...
    const session = await Session.findById(req.params.id).populate('learnerId', 'firstName lastName');
    ...
    // Create a match entry when session is accepted
    const teacher = await User.findById(req.user.id);
    const newMatch = new MatchRequest({
      senderId: session.learnerId._id.toString(),
      senderName: `${session.learnerId.firstName} ${session.learnerId.lastName}`,
      recipientId: session.teacherId.toString(),
      recipientName: `${teacher.firstName} ${teacher.lastName}`,
      status: 'accepted',
      skillOffered: 'Learning Session',
      skillRequested: session.skill
    });
    await newMatch.save();
